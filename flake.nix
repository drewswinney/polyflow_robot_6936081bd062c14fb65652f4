...
EOF
    ...
    # workspace.launch.py MUST exist - fail build if missing
    workspaceLaunchSrc = ./workspace.launch.py;

    rosWorkspace = pkgs.runCommand "polyflow-ros-with-launch" {
      inherit workspaceLaunchSrc;
    } ''
      if [ ! -f "$workspaceLaunchSrc" ]; then
        echo "ERROR: workspace.launch.py not found at repository root" >&2
        echo "This file must be generated by polyflow-studio before deployment" >&2
        exit 1
      fi
      ...
      mkdir -p $out/share
      cp "$workspaceLaunchSrc" $out/share/workspace.launch.py
    '';
